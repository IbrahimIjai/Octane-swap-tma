...
const OctaneSwapLogo: React.FC<OctaneSwapLogoProps> = ({
  ...
  const createOctagonPath = (centerX: number, centerY: number, radius: number) => {
    const points = [];
    for (let i = 0; i < 8; i++) {
  ...
  return (
    <div 
      className={`inline-block ${animated ? 'animate-spin-slow' : ''} ${className}`}
    ...
    >
      ...
      >
        <defs>
          <linearGradient id={`octaneGradient${variant}`} x1="0%" y1="0%" x2="100%" y2="100%">
